...
if (isset($_POST['vssg2_form_submit']) && $_POST['vssg2_form_submit'] == 'yes')
{
	...
	check_admin_referer('vssg2_form_add');
	...
	{
		$sql = $wpdb->prepare(
			"INSERT INTO `".WP_VSSGV2_TABLE."`
			(`vssg_path`, `vssg_link`, `vssg_target`, `vssg_title`, `vssg_order`, `vssg_status`, `vssg_type`)
			VALUES(%s, %s, %s, %s, %d, %s, %s)",
			array($form['vssg_path'], $form['vssg_link'], $form['vssg_target'], $form['vssg_title'], $form['vssg_order'], $form['vssg_status'], $form['vssg_type'])
		);
		$wpdb->query($sql);
		
		$vssg2_success = __('New image details was successfully added.', 'vertical-scroll-slideshow-gallery-v2');
...
if ($vssg2_error_found == FALSE && strlen($vssg2_success) > 0)
{
	?>
	  <div class="updated fade">
		<p><strong><?php echo $vssg2_success; ?> <a href="<?php echo WP_VSSGV2_ADMIN_URL; ?>"><?php _e('Click here', 'vertical-scroll-slideshow-gallery-v2'); ?></a> <?php _e('to view the details', 'vertical-scroll-slideshow-gallery-v2'); ?></strong></p>
	  </div>
	  <?php
	}
?>
...
<div class="form-wrap">
	...
	<form name="vssg2_form" method="post" action="#" onsubmit="return vssg2_submit()"  >
      <h3><?php _e('Add new image details', 'vertical-scroll-slideshow-gallery-v2'); ?></h3>
      <label for="tag-image"><?php _e('Enter image path (URL)', 'vertical-scroll-slideshow-gallery-v2'); ?></label>
      <input name="vssg_path" type="text" id="vssg_path" value="" size="90" />
	  <input type="button" name="upload-btn" id="upload-btn" class="button-secondary" value="Upload Image">
      <p><?php _e('Select and upload your image.', 'vertical-scroll-slideshow-gallery-v2'); ?> (ex: http://www.gopiplus.com/work/wp-content/uploads/pluginimages/250x167/250x167_2.jpg)</p>
      <label for="tag-link"><?php _e('Enter target link', 'vertical-scroll-slideshow-gallery-v2'); ?></label>
      <input name="vssg_link" type="text" id="vssg_link" value="#" size="90" />
      ...
      <input name="vssg_title" type="text" id="vssg_title" value="" size="90" />
      ...
      <input name="vssg_order" type="text" id="vssg_order" size="10" value="1" maxlength="3" />
	  ...
	  <?php wp_nonce_field('vssg2_form_add'); ?>
    </form>
</div>
</div>